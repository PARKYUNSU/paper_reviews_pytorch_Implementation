...
class DwSepConv(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, dilation=1):
        ...
        self.depthwise = BasicConv(in_channels, in_channels, kernel_size=3, stride=stride, padding=dilation, groups=in_channels, dilation=dilation)
...
class EntryFlow(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f"Low-level features shape (after Conv3): {low_level_features.shape}")
...
class ExitFlow(nn.Module):
    def __init__(self, ef_dilation):
        ...
        self.conv1_residual = nn.Sequential(
            ...
            nn.BatchNorm2d(1024),
            nn.ReLU(inplace=True),
            DwSepConv(1024, 1024, dilation=ef_dilation[0]),
            nn.BatchNorm2d(1024),
            DwSepConv(1024, 1024, stride=2, dilation=ef_dilation[1])
        )
...
if __name__ == "__main__":
    model = Xception(output_stride=8)
